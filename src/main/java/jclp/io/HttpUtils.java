...
public final class HttpUtils {
    ...
    public static String makeQuery(Object[] params, String encoding) {
        if (params.length == 0) {
            return "";
        }
    ...
    @SneakyThrows(UnsupportedEncodingException.class)
    public static String makeQuery(Map<String, ?> params, String encoding) {
    ...
    public static class Request {
        ...
        public URLConnection connect() throws IOException {
            String url = this.url;
            Validate.checkNotEmpty(url, "url cannot be null or empty");
            boolean isHttp = url.startsWith("http");
            if (isHttp && CollectionUtils.isNotEmpty(parameters)) {
                url += '?' + HttpUtils.makeQuery(parameters, StringUtils.isNotEmpty(encoding) ? encoding : "UTF-8");
            }
        ...
        private void performHttp(HttpURLConnection conn) throws ProtocolException {
            conn.setRequestMethod(StringUtils.isNotEmpty(method) ? method.toUpperCase() : "GET");
        }
