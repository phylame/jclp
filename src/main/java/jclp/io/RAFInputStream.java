...
class RAFInputStream extends InputStream {
    ...
    RAFInputStream(RandomAccessFile source, long size) throws IOException {
        this(source, source.getFilePointer(), size);
    }

    // size < 0 to use all content of source
    RAFInputStream(@NonNull RandomAccessFile source, long offset, long size) throws IOException {
        this.source = source;
